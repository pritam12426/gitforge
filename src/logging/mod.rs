...
//! value is in exactly those things. What we *did* want from this rewrite
...
//!   * an **environment-variable override** (`GITFORGE_LOG_LEVEL`), so the
//!     verbosity can be bumped for a single run without touching the CLI
//!     invocation — handy when gitforge is launched by another program
//!     (an MCP client, systemd, etc) that doesn't pass `--log-level`
//!     through;
...
//! when disabled — see the crate-level docs in the old `log.rs` for why
//! Cargo features are the right tool here.
...
impl LogLevel {
	/// Parses a level from a string (used for the `GITFORGE_LOG_LEVEL`
...
///
/// `GITFORGE_LOG_LEVEL` in the environment (values: off/fatal/error/warn/
...
pub fn log_init(file_path: Option<&str>, level: LogLevel, format: LogFormat) {
	let level = std::env::var("GITFORGE_LOG_LEVEL")
		.ok()
...
/// Truncates a `Display`-able value for safe inclusion in a log line —
/// tool arguments and results can be arbitrarily large (a full diff, a
/// long commit message), and logging them in full would both bloat the
/// logs and make single-line JSON grep-ability worse. Full detail is
/// still available at `debug`/`trace` level via callers that choose to
/// bypass this.
